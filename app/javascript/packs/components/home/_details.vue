  ...
  <v-layout row wrap>
    <v-flex md4 mb-3 mt-2 offset-md1 xs10 offset-xs1>
      ...
      <v-layout row wrap mt-4>
        <v-flex sm3 md3>
          <v-btn color="red" text-color="white" large class="white--text" :to="'/watch/' + watchable.id">Assistir</v-btn>
        </v-flex>
    ...
    <v-flex md5 offset-md1 hidden-sm-and-down>
      <v-layout row wrap mt-12 center xs-10>
        <img :src="watchable.attributes.thumbnail_cover_url" alt="movie thumbnail" class="responsive-img"/>
      </v-layout>
    </v-flex>
  </v-layout>
</template>
